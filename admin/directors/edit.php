...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '') {
        ...
        $errors[] = 'Director name must be 100 characters or fewer.';
    } else {
        $currentPhoto = $director['photo_path'];
        ...
        if (!empty($_FILES['photo']['name'])) {
            ...
            if ($file['error'] !== UPLOAD_ERR_OK) {
                ...
                $uploadDir = upload_path('uploads/directors/');
                ...
                if (!move_uploaded_file($file['tmp_name'], rtrim($uploadDir, '/\\') . DIRECTORY_SEPARATOR . $filename)) {
                    ...
                    $newPhotoPath = 'uploads/directors/' . $filename;
                }
            }
        }
        ...
        if (empty($errors)) {
            try {
                $pdo->prepare("UPDATE directors SET name = :n, photo_path = :p WHERE id = :id")
                ...
                if ($newPhotoPath !== $currentPhoto && $currentPhoto && file_exists(upload_path($currentPhoto))) {
                    if (!unlink(upload_path($currentPhoto))) {
                        error_log('Failed to delete old director photo: ' . upload_path($currentPhoto));
                    }
                }
                flash_set('success', 'Director updated.');
                redirect('/admin/directors/index.php');
...
<h4 class="fw-bold mb-4"><i class="bi bi-pencil-square"></i> Edit Director: <?= e($director['name']) ?></h4>
...
<form method="post" enctype="multipart/form-data">
    <div class="card mb-4">
        <div class="card-header fw-semibold"><i class="bi bi-info-circle me-2"></i>Director Info</div>
        <div class="card-body">
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <?php if ($director['photo_path']): ?>
                    <div class="mb-2">
                        <img src="<?= e(imageUrl($director['photo_path'], 'director')) ?>"
    ...
    <div class="d-flex gap-2 mb-5">
        ...
        <a href="/admin/directors/index.php" class="btn btn-outline-secondary">Cancel</a>
    </div>
</form>
